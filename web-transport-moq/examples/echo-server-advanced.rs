...
async fn main() -> anyhow::Result<()> {
    ...
    // Standard Noq setup
    let mut config =
        rustls::ServerConfig::builder_with_provider(web_transport_moq::crypto::default_provider())
            .with_protocol_versions(&[&rustls::version::TLS13])?
            .with_no_client_auth()
            .with_single_cert(chain, key)?;

    config.max_early_data_size = u32::MAX;
    config.alpn_protocols = vec![web_transport_moq::ALPN.as_bytes().to_vec()]; // this one is important

    let config: noq::crypto::rustls::QuicServerConfig = config.try_into()?;
    let config = noq::ServerConfig::with_crypto(Arc::new(config));

    tracing::info!(addr = %args.addr, "listening");

    let server = noq::Endpoint::server(config, args.addr)?;
...
async fn run_conn(conn: noq::Incoming) -> anyhow::Result<()> {
    ...
    let request = web_transport_moq::Request::accept(conn).await?;
