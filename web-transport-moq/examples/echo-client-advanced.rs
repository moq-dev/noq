...
async fn main() -> anyhow::Result<()> {
    ...
    let mut config =
        rustls::ClientConfig::builder_with_provider(web_transport_moq::crypto::default_provider())
    ...
    let config = noq::ClientConfig::new(Arc::new(config));

    let client = noq::Endpoint::client("[::]:0".parse()?)?;
    let client = web_transport_moq::Client::new(client, config);
