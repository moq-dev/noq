...
impl Connecting {
    pub async fn accept(conn: &noq::Connection) -> Result<Self, ConnectError> {
...
impl Connected {
    ...
    pub fn session_id(&self) -> VarInt {
        // We gotta convert from the Noq VarInt to the (forked) WebTransport VarInt.
        // We don't use the noq::VarInt because that would mean a noq dependency in
        // web-transport-proto
        let stream_id = noq::VarInt::from(self.send.id());
