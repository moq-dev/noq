...
impl SendStream {
    ...
    /// Wait until the stream has been stopped and return the error code. See
    /// [`noq::SendStream::stopped`].
    ///
    /// Unlike Noq, this returns None if the code is not a valid WebTransport error code.
    /// Also unlike Noq, this returns a SessionError, not a StoppedError, because 0-RTT is not
    /// supported.
    ...
    /// Mark the stream as finished, such that no more data can be written. See
    /// [`noq::SendStream::finish`].
    ///
    ...
    /// > **Warning**
    /// >
    /// > WebTransport sessions share the QUIC connection with HTTP/3 and potentially other
    /// > sessions.
